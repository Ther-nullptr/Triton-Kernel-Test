...
def torch_hadamard(x, y):
    return x * y

@triton.autotune(
    configs=[
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 256, 'GROUP_SIZE_M': 8}, num_stages=3,
                      num_warps=8),
        triton.Config({'BLOCK_SIZE_M': 256, 'BLOCK_SIZE_N': 256, 'GROUP_SIZE_M': 8}, num_stages=2,
                      num_warps=8),
        triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 64, 'GROUP_SIZE_M': 1}, num_stages=8,
                      num_warps=4),
        triton.Config({'BLOCK_SIZE_M': 128, 'BLOCK_SIZE_N': 128, 'GROUP_SIZE_M': 8}, num_stages=4,
                      num_warps=4),
    ],
    key=['M', 'N'],
)
@triton.jit
def triton_hadamard_kernel(
    ...
    a_ptrs = a_ptr + (offs_b * stride_b + offs_m[:, None] * stride_m + offs_n[None, :] * stride_n)
    b_ptrs = b_ptr + (offs_b * stride_b + offs_m[:, None] * stride_m + offs_n[None, :] * stride_n)
    
    # tiling hadamard
    c = tl.zeros(shape=(BLOCK_SIZE_M, BLOCK_SIZE_N), dtype=tl.bfloat16)
    a = tl.load(a_ptrs)
    b = tl.load(b_ptrs)
    b = a * b
    
    # write back
    c_ptrs = c_ptr + (offs_b * stride_b + offs_m[:, None] * stride_m + offs_n[None, :] * stride_n)
    c_mask = (offs_b < B) & (offs_m[:, None] < M) & (offs_n[None, :] < N)
    tl.store(c_ptrs, b, mask=c_mask)
    
    
def triton_hadamard(a, b):
    B, M, N = a.shape
...
@triton.jit
def triton_hadamard_fuse_decompress_kernel(
    # Pointers to matrices
...
def triton_hadamard_fuse_extraction(l, r, q, s, b, quantize_bit=2):
    M, K = l.shape
    K, N = r.shape
    B, _, _ = q.shape
    ...
    triton_hadamard_fuse_decompress_kernel[grid](
        l, r, c, b, a_temp, q, s,
        B, M, N, K,
...
def triton_hadamard_add_extraction(l, r, q, s, b, quantize_bit=2):
    M, K = l.shape
    K, N = r.shape
    B, _, _ = q.shape
    elem_per_position = 8 // quantize_bit
...
c = torch_hadamard(a, b)
triton_c = triton_hadamard(a, b)
...
def benchmark(M, N, provider):
    ...
    if provider == "cublas":
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch_hadamard(a, b), quantiles=quantiles)
    if provider == 'triton':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_hadamard(a, b), quantiles=quantiles)
    if provider == 'triton-fuse':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_hadamard_fuse_extraction(l, r, q, s, b, quantize_bit), quantiles=quantiles)
    if provider == 'triton-no-fuse':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_hadamard_add_extraction(l, r, q, s, b, quantize_bit), quantiles=quantiles)
    perf = lambda ms: 2 * B * M * N * 1e-12 / (ms * 1e-3)
    return perf(ms), perf(max_ms), perf(min_ms)


benchmark.run(print_data=True, show_plots=False, save_path="./hadamard")