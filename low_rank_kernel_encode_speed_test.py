...
from low_rank_extraction_fuse_compression_quantization import low_rank_addition_fuse_compression_quantization, torch_low_rank_addition_fuse_compression_quantization
...
def benchmark(M, provider, fp8_inputs):
    ...
    if provider == 'cublas-w-tail':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch_low_rank_addition_fuse_compression_quantization(l, r, x, s, quantize_bit, outlier), quantiles=quantiles)
    ...
    if provider == 'triton-w-tail':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: low_rank_addition_fuse_compression_quantization(l, r, x, s, quantize_bit, outlier), quantiles=quantiles)
    perf = lambda ms: 2 * B * M * R * M * 1e-12 / (ms * 1e-3)
    return perf(ms), perf(max_ms), perf(min_ms)


benchmark.run(show_plots=False, print_data=True, save_path="./task2")