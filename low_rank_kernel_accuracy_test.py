...
def naive_quantization_dequantization(l, r, x, s, quantize_bit=8, outlier=5.):
    ...
    s = s.unsqueeze(-2)
    q = torch.clamp(torch.round(x / s), min = -2 ** (quantize_bit-1), max = 2 ** (quantize_bit-1) - 1).to(torch.int8)
...
if __name__ == '__main__':
    ...
    x_copy = deepcopy(x)
    s = torch.zeros(1, M, device='cuda', dtype=torch.bfloat16) + 1e-5
    s[:, 300] = 2e-5
    ...
    x_diff = (x_decode - x_baseline) / (x_baseline + 1e-5)
    print(x_decode)
    print(x_baseline)
    print(x_diff)
